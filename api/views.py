...
class ClienteViewSet(ModelViewSet):
    ...
    permission_classes = [
        IsAuthenticated
    ]  # Ajusta las clases de permisos según tus necesidades
...
class FacturaClienteViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        AuditLog.objects.create(
            ...
            details=f"FacturaCliente ID: {factura.id}",
        )
    ...
    def perform_destroy(self, instance):
        AuditLog.objects.create(
            user=self.request.user,
            action="FacturaCliente eliminada",
            details=f"FacturaCliente ID: {instance.id}",
        )
        instance.delete()
...
class FacturaProveedorViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        AuditLog.objects.create(
            ...
            details=f"FacturaProveedor ID: {factura.id}",
        )
    ...
    def perform_destroy(self, instance):
        AuditLog.objects.create(
            user=self.request.user,
            action="FacturaProveedor eliminada",
            details=f"FacturaProveedor ID: {instance.id}",
        )
        instance.delete()
...
from .serializers import (
    ReporteFacturaSerializer,
)  # Asegúrate de importar el serializador ReporteFacturaSerializer
...
class TotalPorCobrarView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        total_por_cobrar = Factura_Cliente.objects.filter(estado="pendiente").aggregate(
            total=Sum("monto")
        )["total"]
...
class TotalPorPagarView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        total_por_pagar = Factura_Proveedor.objects.filter(
            estado="pendiente"
...
class FacturasVencidasView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        hoy = datetime.now()
        facturas_vencidas_clientes = Factura_Cliente.objects.filter(
            fecha_vencimiento__lt=hoy, estado="pendiente"
        )
        facturas_vencidas_proveedores = Factura_Proveedor.objects.filter(
            fecha_vencimiento__lt=hoy, estado="pendiente"
        )
        return Response(
            {
...
class ProyeccionFlujoCajaView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not fecha_inicio or not fecha_fin:
            return Response(
                {"error": "Debe proporcionar fecha_inicio y fecha_fin"}, status=400
            )
        ...
        ingresos = (
            Factura_Cliente.objects.filter(fecha__range=(fecha_inicio, fecha_fin))
        ...
        egresos = (
            Factura_Proveedor.objects.filter(fecha__range=(fecha_inicio, fecha_fin))
            .annotate(dia=TruncDay("fecha"))
            .values("dia")
            .annotate(total=Sum("monto"))
            .order_by("dia")
...
class TotalPorCobrarView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        total_por_cobrar_clientes = (
            Factura_Cliente.objects.filter(estado="pendiente").aggregate(
                total=Sum("monto")
            )["total"]
            or 0
        )
        total_por_cobrar_proveedores = (
            Factura_Proveedor.objects.filter(estado="pendiente").aggregate(
                total=Sum("monto")
            )["total"]
            or 0
        )
...
class FacturasVencidasView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        total_facturas_vencidas = (
            facturas_vencidas_clientes + facturas_vencidas_proveedores
        )
...
class ProyeccionFlujoCajaView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        ingresos = Factura_Cliente.objects.filter(
            fecha__range=(fecha_inicio, fecha_fin)
        ).aggregate(total=Sum("monto"))["total"]
        egresos = Factura_Proveedor.objects.filter(
            fecha__range=(fecha_inicio, fecha_fin)
        ).aggregate(total=Sum("monto"))["total"]
        ...
        return Response(
            {"ingresos": ingresos, "egresos": egresos, "flujo_caja": flujo_caja}
        )
...
class NotificacionesFacturasView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        hoy = timezone.now().date()
        proximas_vencer = Factura_Cliente.objects.filter(
            fecha_vencimiento__lte=hoy + timedelta(days=3), estado="pendiente"
        ).count()
        vencidas = Factura_Cliente.objects.filter(
            fecha_vencimiento__lt=hoy, estado="pendiente"
        ).count()
        return Response({"proximas_vencer": proximas_vencer, "vencidas": vencidas})
...
class TotalFacturasPorEstadoView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        total_facturas_cliente = Factura_Cliente.objects.values("estado").annotate(
            total=Count("estado")
        )
        total_facturas_proveedor = Factura_Proveedor.objects.values("estado").annotate(
            total=Count("estado")
        )
        return Response(
            {
...
class TotalMontoPorEstadoView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        total_monto_cliente = Factura_Cliente.objects.values("estado").annotate(
            total=Sum("monto")
        )
        total_monto_proveedor = Factura_Proveedor.objects.values("estado").annotate(
            total=Sum("monto")
        )
        return Response(
            {
...
class FacturasPorFechaView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        facturas_cliente = Factura_Cliente.objects.filter(
            fecha__range=(fecha_inicio, fecha_fin)
        )
        facturas_proveedor = Factura_Proveedor.objects.filter(
            fecha__range=(fecha_inicio, fecha_fin)
        )
        ...
        if tipo == "cliente":
            for _, row in df.iterrows():
                ...
                if "fecha_vencimiento" in row_dict:
                    ...
                    if (
                        fecha_vencimiento.tzinfo is None
                        or fecha_vencimiento.tzinfo.utcoffset(fecha_vencimiento) is None
                    ):
                        row_dict["fecha_vencimiento"] = timezone.make_aware(
                            fecha_vencimiento
                        )
                try:
...
class ExportarTodasFacturasPDFView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response[
            "Content-Disposition"
        ] = f"attachment; filename=todas_facturas_{tipo}.pdf"
        p = canvas.Canvas(response, pagesize=letter)
        width, height = letter
...
class ExportarTodasFacturasExcelView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response[
            "Content-Disposition"
        ] = f"attachment; filename=todas_facturas_{tipo}.xlsx"
        df.to_excel(response, index=False)
        return response
...
class ExportarFacturaPDFView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response[
            "Content-Disposition"
        ] = f"attachment; filename=factura_{tipo}_{factura_id}.pdf"
        p = canvas.Canvas(response, pagesize=letter)
        width, height = letter
...
class ExportarFacturaExcelView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response[
            "Content-Disposition"
        ] = f"attachment; filename=factura_{tipo}_{factura_id}.xlsx"
        df.to_excel(response, index=False)
        return response
...
class ActualizarEstadoFacturaView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"status": "success", "mensaje": "Estado actualizado correctamente"}
            )
...
class FacturasPerdiendoPlataView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        facturas_cliente = (
            Factura_Cliente.objects.filter(estado="pendiente")
        ...
        facturas_proveedor = (
            Factura_Proveedor.objects.filter(estado="pendiente")
            .annotate(mes=TruncMonth("fecha"))
            .values("mes", "estado")
            .annotate(total=Sum("monto"))
            .order_by("mes")
...
class FacturasGanandoPlataView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        facturas_cliente = (
            Factura_Cliente.objects.filter(estado="pagada")
            .annotate(mes=TruncMonth("fecha"))
            .values("mes", "estado")
            .annotate(total=Sum("monto"))
            .order_by("mes")
        ...
        facturas_proveedor = (
            Factura_Proveedor.objects.filter(estado="pagada")
            .annotate(mes=TruncMonth("fecha"))
            .values("mes", "estado")
            .annotate(total=Sum("monto"))
            .order_by("mes")
...
def register_user(request):
    ...
    if special_password != SPECIAL_PASSWORD:
        return Response(
            {"error": "Contraseña especial incorrecta"},
...
def editar_factura_cliente(request, pk):
    try:
        ...
        return Response(
            {"error": "Factura no encontrada"}, status=status.HTTP_404_NOT_FOUND
        )
...
def editar_factura_proveedor(request, pk):
    ...
    if request.method == "PUT":
        ...
        serializer = FacturaProveedorSerializer(
